...
ggsave(paste0(chm,"/publi/_alti_thm_hist.png"),g.hist.thm, width = 16, height = 10, units ="cm")
```
...
f.thm.z.boxplot <- function(df, name.out){
  ...
  #
  g.bx.thm <- ggplot(aes(y = z, x = thm_xt), data = df_sp)+
...
f.thm.z.boxplot(thm_xt, "_alti_detailed_thm_boxplot.png") # all theme

# main themes
patts <- c("estrella", "arboriforme", "pentacle", "zig−zag",
           "dona", "home",
           "ballesta", "fletxa", "llança", "espasa",
           "cornamusa",
           "ocell", "cavall")
df.patts <- thm_xt[0,]
for (patt in patts){
...
f.thm.z.boxplot(df.patts, "_alti_main_thm_boxplot.png") # all theme
